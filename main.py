...
class CustomContextMenu(ctk.CTkToplevel):
    def __init__(self, parent, command=None):
        ...
        # 👉 внешний контейнер = рамка
        self.border_frame = ctk.CTkFrame(
            self,
            corner_radius=0,
            fg_color="#555555",  # цвет обводки (серый)
        )
        self.border_frame.pack(padx=0, pady=0)

        # 👉 внутренний контейнер = фон меню
        self.inner_frame = ctk.CTkFrame(
            self.border_frame,
            corner_radius=0,
            fg_color="#2b2b2b",
        )
        self.inner_frame.pack(padx=1, pady=1)  # толщина рамки = 1px
...
class ProgressWindow(ctk.CTkToplevel):
    def __init__(self, parent, scanner, on_complete):
        ...
        self.label = ctk.CTkLabel(self, text="Выполняется сканирование...", 
        ...
        self.percent_label = ctk.CTkLabel(self, text="0% (0/0)", font=("Segoe UI", 12))
        self.percent_label.pack(pady=(0, 5))
        
        self.cancel_btn = ctk.CTkButton(self, text="Отмена", 
    ...
    def _update_progress(self, current, total):
        ...
        if total > 0:
            ...
            self.after(0, lambda: self._safe_update(fraction, percent, current, total))
    ...
    def _safe_update(self, fraction, percent, current, total):
        ...
        try:
            ...
            self.percent_label.configure(text=f"{percent}% ({current}/{total})")
        except Exception:
            pass
    ...
    def _on_scan_done(self, folder_sizes, file_sizes, file_types):
        if self._cancelled or not self.winfo_exists():
            return
        self.after(0, lambda: self._safe_done(folder_sizes, file_sizes, file_types))
...
class DiskAnalyzerApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        self.path_label = ctk.CTkLabel(self.top_frame, text="Папка: не выбрана", 
                                        anchor="w", width=400)
        self.path_label.pack(side="left", padx=5)
        
        self.browse_btn = ctk.CTkButton(self.top_frame, text="Обзор", 
                                         command=self._browse_folder, width=80)
        self.browse_btn.pack(side="left", padx=5)
        
        self.scan_btn = ctk.CTkButton(self.top_frame, text="Сканировать", 
                                       command=self._start_scan, width=100,
                                       state="disabled")
        self.scan_btn.pack(side="left", padx=5)
        
        self.export_btn = ctk.CTkButton(self.top_frame, text="Экспорт CSV", 
                                         command=self._export_csv, width=100,
                                         state="disabled")
        self.export_btn.pack(side="left", padx=5)
        
        self.status_label = ctk.CTkLabel(self.top_frame, text="Готов", text_color="gray")
        ...
        self.tree_label = ctk.CTkLabel(self.left_frame, text="Дерево папок и файлов:", 
                                        font=("Segoe UI", 14, "bold"))
        ...
        self.path_info_label = ctk.CTkLabel(self.path_info_frame, text="Путь: ", 
                                             anchor="w", font=("Segoe UI", 10))
        self.path_info_label.pack(side="left", padx=(5, 0), pady=2)
        
        self.path_info_value = ctk.CTkLabel(self.path_info_frame, text="", 
                                             anchor="w", font=("Segoe UI", 10, "bold"),
                                             text_color="#4fc3f7")
        self.path_info_value.pack(side="left", padx=5, pady=2, fill="x", expand=True)
        
        self.copy_path_btn = ctk.CTkButton(self.path_info_frame, text="📋", 
        ...
        self.tree_scroll_y = ctk.CTkScrollbar(self.tree_container, orientation="vertical")
        self.tree_scroll_y.pack(side="right", fill="y")
        self.tree.configure(yscrollcommand=self.tree_scroll_y.set)
        self.tree_scroll_y.configure(command=self.tree.yview)
        
        self.tree_scroll_x = ctk.CTkScrollbar(self.tree_container, orientation="horizontal")
        ...
        self.chart_label = ctk.CTkLabel(self.right_frame, text="Круговая диаграмма:", 
                                         font=("Segoe UI", 14, "bold"))
        ...
        self.types_label = ctk.CTkLabel(self.types_frame, text="По типам файлов:", 
                                         font=("Segoe UI", 12, "bold"))
        self.types_label.pack(side="left", padx=10, pady=5)
        
        self.types_text = ctk.CTkLabel(self.types_frame, text="Ожидание сканирования...", 
                                        font=("Consolas", 11))
    ...
    def _start_scan(self):
        ...
        self.status_label.configure(text="Сканирование...", text_color="#f39c12")
        ...
        self.progress_window = ProgressWindow(self, self.scanner, self._on_scan_complete)
